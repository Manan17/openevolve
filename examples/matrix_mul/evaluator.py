...
class MatrixMultiplicationEvaluator:
    """Comprehensive evaluator for Triton matrix multiplication kernels"""
    ...
    def evaluate(self, program_path: str) -> Dict[str, Any]:
        try:
            ...
            if not hasattr(program, 'compute_matmul'):
                return self._create_error_result("Missing compute_matmul function")
            
            if not hasattr(program, 'matmul_kernel'):
                return self._create_error_result("Missing matmul_kernel function")
    ...
    def _test_correctness(self, program) -> Dict[str, float]:
        """Test basic correctness of the matrix multiplication"""
        try:
            # Test cases with different sizes
            test_cases = [
                ((64, 64), (64, 64), torch.float32),
                ((128, 128), (128, 128), torch.float32),
                ((256, 256), (256, 256), torch.float32),
                ((64, 128), (128, 64), torch.float32),
                ((128, 64), (64, 128), torch.float32),
                ((64, 64), (64, 64), torch.float16),
                ((128, 128), (128, 128), torch.float16),
            ]
            
            total_accuracy = 0.0
            num_tests = 0
            
            for (m, k), (k2, n), dtype in test_cases:
                # Generate test data
                ...
                # Run kernel
                try:
                    result = program.compute_matmul(a, b)
                    ...
                    print(f"Correctness test failed for size ({m}, {k}) x ({k}, {n}), dtype {dtype}: {e}")
                    continue
    ...
    def _test_performance(self, program) -> Dict[str, float]:
        ...
        try:
            ...
            for (m, k), (k2, n) in test_sizes:
                ...
                # Warm up
                for _ in range(3):
                    _ = program.compute_matmul(a, b)
                
                # Benchmark baseline (PyTorch)
                torch.cuda.synchronize()
                start_time = time.time()
                for _ in range(num_trials):
                    baseline_result = torch.mm(a, b)
                torch.cuda.synchronize()
                baseline_time = (time.time() - start_time) / num_trials
                baseline_times[f"{m}x{k}x{n}"] = baseline_time
                
                # Benchmark kernel
                torch.cuda.synchronize()
                start_time = time.time()
                for _ in range(num_trials):
                    kernel_result = program.compute_matmul(a, b)
                torch.cuda.synchronize()
                kernel_time = (time.time() - start_time) / num_trials
                kernel_times[f"{m}x{k}x{n}"] = kernel_time
            
            # Calculate speedup scores
            speedup_scores = []
            for size_key in baseline_times.keys():
                if size_key in baseline_times and size_key in kernel_times:
                    speedup = baseline_times[size_key] / max(kernel_times[size_key], 1e-9)
    ...
    def _test_robustness(self, program) -> Dict[str, float]:
        ...
        try:
            ...
            try:
                a = torch.zeros(64, 64, device=self.device)
                b = torch.zeros(64, 64, device=self.device)
                result = program.compute_matmul(a, b)
                expected = torch.mm(a, b)
                ...
                print(f"Zero matrix test failed: {e}")
            
            # Test 2: Identity matrices
            try:
                a = torch.eye(64, device=self.device)
                b = torch.eye(64, device=self.device)
                result = program.compute_matmul(a, b)
                expected = torch.mm(a, b)
                if torch.allclose(result, expected):
                    robustness_score += 1.0
                ...
                print(f"Identity matrix test failed: {e}")
            
            # Test 3: Large matrices
            try:
            ...
            try:
                a = torch.randn(64, 64, dtype=torch.float16, device=self.device)
                b = torch.randn(64, 64, dtype=torch.float16, device=self.device)
                result = program.compute_matmul(a, b)
                expected = torch.mm(a, b)
            ...
            try:
                ...
                print(f"Non-square matrix test failed: {e}")
    ...
    def _test_memory_efficiency(self, program) -> Dict[str, float]:
        ...
        try:
            ...
            expected_memory = a.element_size() * m * k + b.element_size() * k * n + result.element_size() * m * n
            
            if memory_used <= expected_memory * 2.0:  # Allow 100% overhead for matmul
                memory_score = 1.0
            else:
...
def evaluate_stage1(program_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(program, 'compute_matmul'):
            return {'stage1_score': 0.0, 'combined_score': 0.0, 'error': 'Missing compute_matmul function'}
        
        if not hasattr(program, 'matmul_kernel'):
            return {'stage1_score': 0.0, 'combined_score': 0.0, 'error': 'Missing matmul_kernel function'}
